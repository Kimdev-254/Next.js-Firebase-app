    ...
    <section className="max-w-7xl mx-auto mb-15 text-gray-300 border-y-indigo-500">
      <nav className="bg-card text-card-foreground p-4 shadow-md rounded-lg flex items-center justify-between">
        <div className="flex items-center space-x-10">
          <i
            className="fa fa-file-text transform hover:scale-110 transition-transform duration-300 overflow-hidden"
            aria-hidden="true"
          ></i>
          <div className="hidden md:flex items-center space-x-10">
            <Link legacyBehavior href="/">
              <a
                className={`text-muted-foreground cursor-pointer pb-1 ${
                  router.pathname === "/"
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-card text-card-foreground p-4 rounded-lg shadow-md mt-2">
          <Link legacyBehavior href="/">
            <a
              className={`block text-muted-foreground cursor-pointer pb-1 mb-2 ${
                router.pathname === "/"
                  ? "text-primary border-b-2 border-primary"
                  : "hover:text-primary hover:border-b-2 hover:border-primary"
              }`}
